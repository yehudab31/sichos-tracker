  ...
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        <StatCard icon={<BookMarked size={16} />} label="Sichos"  value={learnedSichos}              sub={`of ${totalSichos.toLocaleString()}`} />
        ...
        <StatCard icon={<BarChart3 size={16} />} label="Overall" value={`${sichosPct}%`}            sub="by sicha count" accent               />
      </div>

      {/* Progress bar — label on the RIGHT (LTR layout, bar left, text right) */}
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-semibold text-[#0B1F3A] whitespace-nowrap tabular-nums">
          {learnedSichos.toLocaleString()} / {totalSichos.toLocaleString()} · {sichosPct}%
        </span>
      </div>
    </section>
  );
